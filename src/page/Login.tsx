...
export function Login() {
  ...
  const [senha, setSenha] = useState(''); // Se o seu FiltroGlobal usar senha
 ...
 localStorage.setItem('usuario', JSON.stringify({
        ...
        fotoPerfil: response.data.fotoPerfil,   // ← aqui
        capaPerfil: response.data.capaPerfil,
        moedasFavoritas: response.data.moedasFavoritas
      }));
      ...
      <div className="coin-card" style={{ 
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <input 
            type="email" 
            placeholder="E-mail" 
            className="search-input" // Usa sua animação de foco verde
          ...
          <button 
            type="submit" 
            className="btn-filtrar" // Usa sua animação azul do botão "Filtrar"
            style={{ 
              padding: '14px', 
