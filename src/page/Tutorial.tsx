...
const passos = [
  {
    ...
    descricao: 'Criptomoedas são moedas digitais que existem apenas na internet. Assim como o real vale dinheiro no Brasil, o Bitcoin vale dinheiro no mundo todo — mas só existe no computador.',
    visual: (
  ...
  },
  {
    ...
    descricao: 'O preço mostra quanto você precisaria pagar em reais para comprar 1 unidade daquela moeda. Você não precisa comprar 1 inteiro — pode comprar uma fração pequena com qualquer valor!',
    visual: (
      <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {[['₿', 'Bitcoin', 'R$ 520.000,00'], ['Ξ', 'Ethereum', 'R$ 18.400,00'], ['◎', 'Solana', 'R$ 780,00']].map(([icon, nome, preco]) => (
        ...
        <p style={{ color: '#666', fontSize: '13px', textAlign: 'center', margin: '4px 0 0' }}>Com R$ 100 você compraria 0,000192 BTC ou 0,0054 ETH</p>
      </div>
    )
  },
  {
    titulo: 'Como ler o gráfico?',
    descricao: 'O gráfico mostra como o preço mudou nos últimos dias. Linha subindo = preço aumentou. Linha descendo = preço caiu. É normal oscilar — isso se chama volatilidade.',
    visual: (
  ...
  },
  {
    ...
    descricao: 'Crie uma conta e clique no botão ⭐ em qualquer moeda para adicioná-la aos seus favoritos. Seus favoritos ficam salvos no seu perfil e você recebe alertas quando o preço variar muito.',
    visual: (
      <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
  ...
  },
  {
    ...
    descricao: 'Depois de favoritar uma moeda, você pode simular quanto receberia investindo um valor. Nenhum dinheiro real é movimentado — é só para aprender e entender melhor o mercado!',
    visual: (
      <div style={{ marginTop: '20px', backgroundColor: '#1e1e1e', borderRadius: '12px', padding: '20px' }}>
        ...
        <input
          type="text"
          defaultValue="R$ 500,00"
          readOnly
          style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #444', backgroundColor: '#121212', color: 'white', marginBottom: '12px', boxSizing: 'border-box' }}
        />
      ...
      <div style={{ width: '100%', maxWidth: '560px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <div>
            ...
            <p style={{ color: '#aaa', fontSize: '14px', margin: '4px 0 0' }}>Passo {atual + 1} de {passos.length}</p>
          </div>
          <button
            onClick={() => navigate('/')}
            style={{ padding: '8px 16px', backgroundColor: '#333', color: '#aaa', border: '1px solid #444', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}
          >
            Pular tutorial
          </button>
        </div>
        ...
        {/* Card do passo */}
        <div style={{ backgroundColor: '#1a1a1a', borderRadius: '16px', padding: '28px', border: '1px solid #2a2a2a', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginBottom: '20px' }}>
          {passos.map((_, i) => (
            <div
              key={i}
              onClick={() => setAtual(i)}
              style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: i === atual ? '#3b82f6' : '#444', cursor: 'pointer', transition: 'background 0.2s' }}
            />
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button
            ...
            disabled={atual === 0}
            style={{ flex: 1, padding: '14px', backgroundColor: '#333', color: atual === 0 ? '#555' : 'white', border: '1px solid #444', borderRadius: '10px', cursor: atual === 0 ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '15px' }}
          >
            ← Anterior
          </button>
          {atual < passos.length - 1 ? (
            <button
              onClick={() => setAtual(a => a + 1)}
              style={{ flex: 2, padding: '14px', backgroundColor: '#3b82f6', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px' }}
            >
              Próximo →
            </button>
          ) : (
            <button
              onClick={() => navigate('/')}
              style={{ flex: 2, padding: '14px', backgroundColor: '#22c55e', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px' }}
            >
              ✅ Começar a usar!
            </button>
          )}
